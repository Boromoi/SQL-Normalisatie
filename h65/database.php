...
class Database
{
    ...
    function __construct() {
        ...
        try {
            ...
            $this->db = new PDO($dsn, $user, $psw, array(PDO::ATTR_PERSISTENT => true));
    ...
    function getAantal($id) {
        $sql = "select count(*) as 'Aantal leden'
				from lid 
				inner join lidcommissie using (lidid)
				inner join commissie using (commissieid)
				where commissieid ='$id'";
    ...
    function getLeden($id) {
        $sql = "select concat_ws(' ', voorletters, tussenvoegsel, achternaam) as naam, telefoon
            from lid
            inner join lidcommissie using (lidid)
            inner join commissie using (commissieid)
            where commissieid = '$id'";
    ...
    function getCommissies() {
        $sql = "select commissieid, commissienaam from commissie";
        return $this->db->query($sql);
    }
    ...
    function getCommissienaam($id) {
        $sql = "select commissienaam from commissie where commissieid = '$id'";
        $result = $this->db->query($sql);
        return $result->fetchColumn(0);
    }
}
